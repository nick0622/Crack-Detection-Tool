...
class CrackDetector:
    def __init__(self, model_type="yolov8_single", enhance_images=False, use_tta=False,
                 use_tiling=False, multi_model_file=None):
        """
        ...
        Args:
            ...
            multi_model_file: Override filename for yolov8_4classes model
                              e.g. "yolov8_multi_highres.onnx"
        """
        self.session = None
        ...
        self.multi_model_file = multi_model_file
        self.load_model()
    ...
    def get_model_path(self):
        ...
        model_mapping = {
            ...
            "yolov8_4classes": self.multi_model_file if self.multi_model_file else "yolov8_multi.onnx",
        }
    ...
    def load_model(self):
        ...
        try:
            ...
            if self.multi_model_file and self.model_type == "yolov8_4classes":
                print(f"📦 Sub-Model: {self.multi_model_file}")
    ...
    def preprocess_image(self, image):
        """Standard YOLOv8 Letterbox preprocessing"""
    ...
    def merge_tiled_detections_simple(self, all_tile_detections, img_width, img_height,
                                      iou_threshold=0.5):
        """
        簡單合併4個tile的偵測結果：對每個class各自做 NMS，去除跨tile重疊框。
        不考慮邊界邏輯，直接合併所有框。
        """
                ...
                for i in indices:
                    ...
                    final_detections.append({
                        "bbox": {
                            "x1": max(0, float(b[0])),
                            "y1": max(0, float(b[1])),
                            "x2": min(img_width, float(b[2])),
                            "y2": min(img_height, float(b[3]))
                        },
    ...
    def detect_cracks_tiling(self, image, confidence=0.25):
        """
        使用4-tile inference偵測裂縫（簡化版：直接 NMS 合併，不處理邊界）
        """
        ...
        # 簡單 NMS 合併，不考慮邊界
        merged_detections = self.merge_tiled_detections_simple(
            all_tile_detections,
...
def main():
    ...
    parser.add_argument("--multi-model-file", type=str, default=None,
                        help="Override ONNX filename for yolov8_4classes "
                             "(e.g. yolov8_multi_highres.onnx). "
                             "File must exist in the model/ directory.")
    parser.add_argument("--tta", "-t", action="store_true",
                        help="Use Multi-Scale Test Time Augmentation (slower but more accurate)")
    parser.add_argument("--tiling", action="store_true",
                        help="Use 4-tile inference (splits image into 2x2 tiles, merged with NMS)")

    args = parser.parse_args()

    if not args.input:
        print("\n📁 Please provide an image file or folder:")
        print("   Example: python inference_yolo.py image.jpg")
        print("   Example: python inference_yolo.py photos/ --enhance --save --model yolov8_4classes --tta")
        print("   Example with tiling: python inference_yolo.py image.jpg --tiling --save")
        print("   Example with sub-model: python inference_yolo.py image.jpg --model yolov8_4classes --multi-model-file yolov8_multi_highres.onnx")
    ...
    detector = CrackDetector(
        ...
        multi_model_file=args.multi_model_file
    )
    ...
    if args.tiling:
        ...
        print("   • Image split into 2x2 tiles, merged with NMS")
    elif args.tta:
